...
function Door({ title, body, cta, variant, onClick, chips }: DoorProps) {
  return (
    ...
    >
      <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 21, letterSpacing: '-0.01em', margin: '0 0 8px', color: 'var(--ink)' }}>{title}</h2>
      <p style={{ fontFamily: 'var(--font-body)', fontSize: 14.5, lineHeight: 1.5, color: 'var(--ink-60)', margin: '0 0 16px' }}>{body}</p>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 20 }}>
        {chips.map((c) => (
          <span
            key={c}
            style={{ fontFamily: 'var(--font-body)', fontSize: 12.5, color: 'var(--ink-60)', background: 'var(--ink-06)', borderRadius: 'var(--radius-pill)', padding: '5px 11px' }}
          >
            {c}
          </span>
        ))}
      </div>
      <Button variant={variant} size="lg" style={{ width: '100%', marginTop: 'auto' }} onClick={onClick}>
        {cta}
      </Button>
    </div>
  )
