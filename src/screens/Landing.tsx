...
export function Landing({ onConnect, onNav }: LandingProps) {
  ...
  return (
    <main>
      {/* Hero */}
      <section className="hb-hero-grid hb-rise" style={{ maxWidth: 1320, margin: '0 auto', padding: '64px 32px 40px' }}>
        <div>
          <div className="hb-eyebrow" style={{ marginBottom: 20 }}>{t('eyebrow')}</div>
          <h1
            style={{
      ...
      <section style={{ maxWidth: 1320, margin: '0 auto', padding: '8px 32px 64px' }}>
        ...
        >
          <div style={counterCell}>
            <StatBlock label={t('poolValue')} value="$4,862,014" decimals=".55" size="lg" href="#" />
          </div>
          <div style={counterCell}>
      ...
      <section id="how" style={{ background: 'var(--surface)', borderTop: '1px solid var(--ink-12)', borderBottom: '1px solid var(--ink-12)', scrollMarginTop: 68 }}>
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: '64px 32px' }}>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 16, color: 'var(--ink-60)', margin: '0 0 36px', maxWidth: 560 }}>{t('howSub')}</p>
          <div className="hb-how-grid">
            {steps.map((i) => (
              <div key={i}>
                ...
                <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, margin: '0 0 6px', color: 'var(--ink)' }}>{t(`step${i}Title`)}</h3>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, lineHeight: 1.5, color: 'var(--ink-60)', margin: 0 }}>{t(`step${i}Body`)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="verify" className="hb-verify-grid" style={{ maxWidth: 1320, margin: '0 auto', padding: '64px 32px 80px', scrollMarginTop: 68 }}>
        <div>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 16, lineHeight: 1.6, color: 'var(--ink-60)', margin: '0 0 16px' }}>{t('verifyBody')}</p>
